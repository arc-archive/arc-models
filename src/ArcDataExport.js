...
/** @typedef {import('@advanced-rest-client/events').ArcExportEvent} ArcExportEvent */
...
export class ArcDataExport {
  ...
  constructor(eventsTarget) {
     ...
     */
    this.electronCookies = undefined;
    /**
  ...
  listen() {
    ...
    eventsTarget.addEventListener(DataExportEventTypes.customData, this[exportHandler]);
    eventsTarget.addEventListener(DataExportEventTypes.nativeData, this[nativeExportHandler]);
  }
  ...
  unlisten() {
    const { eventsTarget } = this;
    eventsTarget.removeEventListener(DataExportEventTypes.customData, this[exportHandler]);
    eventsTarget.removeEventListener(DataExportEventTypes.nativeData, this[nativeExportHandler]);
  }
  ...
  async [exportFile](data, options) {
    ...
    const result = await ExportEvents.fileSave(this.eventsTarget, data, config);
    if (!result) {
  ...
  async [exportDrive](data, options) {
    ...
    const result = await GoogleDriveEvents.save(this.eventsTarget, data, config);
    if (!result) {
  ...
  async [encryptData](data, passphrase) {
    ...
    const encoded = await EncryptionEvents.encrypt(this.eventsTarget, data, passphrase, 'aes');
    if (!encoded) {
